...
Two fairness protections apply on top of that:

- **New stakers are protected.** If your first deposit landed *after* a
  dispute's vote already started, you can't be slashed as a non-voter of
  that dispute, because you never had a chance to participate.
- **Pre-dispute unlocks are protected.** $AURA you requested to unlock
  *before* a dispute started is out of that dispute's slashing reach.
  Requesting the unlock *after* the dispute started doesn't help, since the
  pending amount stays exposed until the round settles, so "vote and
  flee" doesn't work.
...
On top of the slasher gate, the Vault enforces a **global slashing
circuit breaker**: no more than **1% of the entire vault** can be
slashed in any rolling 24-hour window, no matter who asks. Legitimate
dispute penalties never come close to that ceiling (a round slashing
every staker moves ~0.1% of the vault), so the cap exists purely as
defense-in-depth: even a fully compromised oracle could not drain
staked $AURA faster than 1% per day, leaving days of notice to respond.
