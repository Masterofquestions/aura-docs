...
<Note>
Current pre-mainnet source supersedes the July bearer-token entry below:
new sessions are cookie-only, `issueToken` is rejected, and refresh uses the
explicit CSRF-protected session-refresh route. Creator tiers/fee points and
Vault automatic withdrawal now follow their current guides. Historical entries
are preserved as development history, not deployed-client obligations.
</Note>
