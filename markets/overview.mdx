...
description: "How Aura prediction markets work: outcome shares, the dual order book, how markets are created, and how outcomes are settled."
keywords: ["how prediction markets work", "Aura markets", "yes no shares", "order book", "market resolution", "binary market"]
---

An Aura market asks a clearly defined question with two outcomes, or is part of a
linked event page containing several options. You buy shares in the outcome you
think is right, and if you are right, each share pays out \$1. Aura does not stop
trading because a calendar date passes; an outcome can be proposed whenever the
event is decidable under the market's rules.
...
<Steps>
  <Step title="Someone proposes it">
    A creator writes the question, the resolution rules, and the source that will
    settle it. Proposing uses capacity backed by active stake, either personally
    held or supplied through an accepted borrowed slot. See
  ...
  <Step title="Someone proposes the outcome">
    When the event is decidable under the market's rules, anyone can submit the
    outcome with a 100 USDT bond. If nobody disputes it within 24 hours, it
    stands.
  </Step>
  ...
  <Step title="Winnings are delivered">
    Once the outcome is final and settlement conditions are met, winning shares
    pay \$1 each through automatic delivery. Referral and voter rewards follow
    their own settlement paths. Creator rewards use a separate daily schedule
    that can pay before resolution; see [Creator rewards](/programs/creator-rewards).
  </Step>
</Steps>
